...
export default function Transactions() {
  const columns = [
    ...
    },
    {
      ...
      render: (category: string) => {
        const color = category === 'Air Travel' ? 'blue' : 
                      category === 'Hotel' ? 'green' : 
                      category === 'Ground Transport' ? 'orange' : 'purple';
  ...
  const topSpendersColumns = [
    ...
    },
    {
      ...
      key: 'status',
      render: (status: string) => {
        const color = status === 'Within Budget' ? 'green' : 
                      status === 'Near Limit' ? 'orange' : 'red';
        return <Tag color={color}>{status}</Tag>;
      },
    },
  ];
